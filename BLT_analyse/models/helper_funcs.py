...
def get_network_model(hyp):
    ...
    if hyp['network']['model'] == 'BLT_net':
        ...
        if hyp['dataset']['image_size'] == 64:
            ...
            net = BLT_net_128(lateral_connections = l_flag, topdown_connections = t_flag, LT_interaction = ltinteract, timesteps = T_info, LT_position = ltpos)
            net_name = f'b128_l_{l_flag}_t_{t_flag}_ltinteract_{ltinteract}_T_{T_info}_ltposition_{ltpos}_num_{netnum}'
            print(f'\nNetwork name: {net_name}')        
...
def eval_network(data_loader,net,hyp): 
